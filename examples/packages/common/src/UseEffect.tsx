 ...
 *
 * Scenario:
 * Let's say, you have a component and you want to toggle visibility
 * externally depends on some condition. This use case shows using
 ...
 * When you do not use the state in deps, the `state.num` will be always the initial value.
 * To fix this issue, you can provide an ubsubscribe action, which will remove
 * the created object from `listeners` when useEffect unmounts. Or you
 * can use useRef. Pass the state to the useRef object on each render and use
 * it instead of `state.num` like this: `ref.current.num`. UseRef always
 * returns the same object so in the `ref.current.num` will be always the
 * actual number.
 */
 ...
 * The second way is to use an unmount and delete added object from previous render.
